...
void RA_NET::WebResponse(const prog_char* response, long strsize)
{
  PrintHeader(strsize,0);
  PROGMEMprint(response);
}

void RA_NET::ModeResponse(bool fOk)
{
  uint8_t s = ( fOk ) ? 15 : 16;
...
void RA_NET::PushBuffer(byte inStr)
{
...
//#endif  // DisplayLEDPWM
        ...
        else if ( !bHasSecondValue && (weboption2 >= 0) && (bCommaCount==0) )
        {
          ...
          PROGMEMprint(XML_CLOSE_TAG);
          if ( reqtype == REQ_M_BYTE )
            print(InternalMemory.read(newweboption2),DEC);
          else
            print(InternalMemory.read_int(newweboption2),DEC);
          PROGMEMprint(XML_M_CLOSE);
          print(weboption2, DEC);
          PROGMEMprint(XML_CLOSE_TAG);
        }
...
void RA_NET::PrintHeader(int s, byte type)
{
...
char RA_NET::GetC(int c)
{
  return pgm_read_byte(c+EncodingChars);
}

void RA_NET::ConvertC(char* strIn, char* strOut, byte len)
{
...
void RA_NET::Authentication(char* userpass)
{
  char* authPtr;
...
void RA_NET::SendXMLData(bool fAtoLog)
{
...
void RA_NET::PROGMEMprint(const prog_char str[])
{
  char c;
  if(!str) return;
...
void RA_NET::LoadWebBanner(int pointer, byte qty)
{
  //  webbannerpointer = pointer;
  //  webbannerqty = qty;
}

void RA_NET::Portal(char *username)
{
//  if (ReefAngel.Timer[PORTAL_TIMER].IsTriggered()) SendPortal(username,"");
//  portalusername=username;
}

void RA_NET::Portal(char *username, char *key)
{
//  if (ReefAngel.Timer[PORTAL_TIMER].IsTriggered()) SendPortal(username,key);
//  portalusername=username;
}

void RA_NET::SendPortal(char *username, char*key)
{
